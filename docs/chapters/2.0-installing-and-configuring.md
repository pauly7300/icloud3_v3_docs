...
> Every time iCloud3 starts, it checks to see if the latest version is installed and, if not, copies the new version to the *config/www/icloud3* directory. An alert notice is displayed in the Event Log that the browser's cache needs to be cleared (see below) before it can be used. 

The default directory for the Event Log Card is the *config/www/icloud3* directory. it can be changee it to another location (e.g.: *www/community/icloud3*) on the *iCloud3 Configurator > Menu Page 2 > Event Log Configuration* screen. 
