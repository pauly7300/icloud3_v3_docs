...
- **iCloud Event Log Card Lovelace Resources Directory**- The Event Log Card File (*icloud3-event-log-card.js*) file is stored in this directory. When iCloud3 is started the first time, the *www/icloud3* directory is created and the the *.js* file is copied into it. To use another directory, change the name here. When iCloud3 restarts, the Event Log file is copied to that directory and the Lovelace Resource entry is updated.
